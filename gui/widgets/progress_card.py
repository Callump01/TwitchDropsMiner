...
class ProgressCard(AnimatedCard):
    """
    Campaign and drop progress card with animated gradient progress bars.

    Shows: game name, campaign name, campaign progress bar,
    drop rewards, drop progress bar, and countdown timers.
    """

    ALMOST_DONE_SECONDS = 10
    PROGRESS_MAX = 10000  # Use integer range for QProgressBar
    ...
    def __init__(self, manager, parent=None):
        ...
        self._campaign_bar.setFixedHeight(8)
        self.card_layout.addWidget(self._campaign_bar)
        ...
        self.card_layout.addWidget(self._campaign_remaining)
        ...
        self.card_layout.addWidget(drop_title)

        self._drop_rewards = QLabel("...", self)
        self.card_layout.addWidget(self._drop_rewards)

        drop_header = QHBoxLayout()
        drop_prog_label = QLabel(_("gui", "progress", "drop_progress"), self)
        drop_prog_label.setProperty("class", "muted")
        drop_header.addWidget(drop_prog_label)
        drop_header.addStretch(1)
        self._drop_pct = QLabel("-%", self)
        drop_header.addWidget(self._drop_pct)
        self.card_layout.addLayout(drop_header)
        ...
        self._drop_bar.setFixedHeight(8)
        self.card_layout.addWidget(self._drop_bar)

        self._drop_remaining = QLabel("", self)
        self._drop_remaining.setProperty("class", "muted")
        self._drop_remaining.setAlignment(Qt.AlignmentFlag.AlignRight)
        self.card_layout.addWidget(self._drop_remaining)

        # Smooth progress animators
